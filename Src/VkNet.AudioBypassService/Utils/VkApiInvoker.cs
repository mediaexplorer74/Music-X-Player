...
namespace VkNet.AudioBypassService.Utils
{
	...
	public class VkApiInvoker : IVkApiInvoker
	{
		...
		public VkApiInvoker([NotNull] IVkApiVersionManager versionManager, 
			[NotNull] IRestClient restClient, [NotNull] ICaptchaHandler captchaHandler)
		{
		...
		public async Task<T> CallAsync<T>(Uri uri, VkParameters parameters, 
		...
		public async Task<JToken> CallAsync(Uri uri, VkParameters parameters, 
			CancellationToken cancellationToken = default)
		{
		...
		public async Task<T> CallAsync<T>(string methodName, VkParameters parameters, 
			CancellationToken cancellationToken = default)
		{
			var url = $"https://api.vk.com/method/{methodName}";

			var jObject = await CallAsync(new Uri(url), parameters, cancellationToken)
		...
		public Task<JToken> CallAsync(string methodName, VkParameters parameters, 
			CancellationToken cancellationToken = default)
		{
			return CallAsync<JToken>(methodName, parameters, cancellationToken);
		}

		internal async Task<string> InvokeAsyncInternal(Uri uri, IDictionary<string, string> parameters,
			CancellationToken cancellationToken = default)
		{
