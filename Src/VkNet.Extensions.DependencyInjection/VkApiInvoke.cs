...
public class VkApiInvoke : IVkApiInvoke
{
    ...
    private void TryAddRequiredParameters(IDictionary<string, string> parameters, bool skipAuthorization)
    {
        ...
        */
        ...
        if (_versionManager.Version != null)
          parameters.Add("v", _versionManager.Version);

        if (_languageService.GetLanguage()?.ToString() != null)
        parameters.Add("lang", _languageService.GetLanguage()?.ToString() ?? "ru");

        if (!skipAuthorization)
        {
            if (_tokenStore.Token != null)
              parameters.Add("access_token", _tokenStore.Token);
        }
    }
    ...
    {
        return CallAsync<T>(methodName, parameters, skipAuthorization, jsonConverters)
            .GetAwaiter().GetResult();
    }

    private async Task<T?> CallAsync<T>(string methodName, VkParameters parameters,
        bool skipAuthorization, JsonConverter[] jsonConverters)
    {
    ...
    public string Invoke(string methodName, IDictionary<string, string> parameters, 
        bool skipAuthorization = false)
    {
    ...
    {
        return CallAsync<string>(methodName, default, skipAuthorization, Array.Empty<JsonConverter>());
    }
